...
let Mandelbrot = {
  ...
  get: function get(data) {
    ...
    for (let ix = data.startX; ix < data.startX + data.stepX; ++ix) {
      for (let iy = data.startY; iy < data.startY + data.stepY; ++iy) {
        let globalX = minX + (maxX - minX) * ix / (data.width - 1);
        let globalY = minY + (maxY - minY) * iy / (data.height - 1);
        let rawPixelPosition = 4 * (data.stepX * (iy - data.startY) + ix - data.startX);
